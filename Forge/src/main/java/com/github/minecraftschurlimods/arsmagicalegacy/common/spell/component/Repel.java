...
public class Repel extends AbstractComponent {
    ...
    private static SpellCastResult performRepel(@Nullable Entity targetEntity, AABB aabb, LivingEntity caster, Vec3 targetPos, List<ISpellModifier> modifiers, ISpell spell, HitResult target) {
    ...
    public SpellCastResult invoke(ISpell spell, LivingEntity caster, Level level, List<ISpellModifier> modifiers, BlockHitResult target, int index, int ticksUsed) {
        float range = ArsMagicaAPI.get().getSpellHelper().getModifiedStat(2, SpellPartStats.RANGE, modifiers, spell, caster, target) * 2;
        return performRepel(null, AABB.ofSize(target.getLocation(), range, range, range), caster, target.getLocation(), modifiers, spell, target);
    }
}
