...
public interface AMRegistries {
    ...
    DeferredRegister<ISkillPoint>         SKILL_POINTS       = DeferredRegister.create(ISkillPoint.REGISTRY_KEY,              ArsMagicaAPI.MOD_ID);
    DeferredRegister<IAffinity>           AFFINITIES         = DeferredRegister.create(IAffinity.REGISTRY_KEY,                ArsMagicaAPI.MOD_ID);
    DeferredRegister<ISpellPart>          SPELL_PARTS        = DeferredRegister.create(ISpellPart.REGISTRY_KEY,               ArsMagicaAPI.MOD_ID);
    DeferredRegister<IAbility>            ABILITIES          = DeferredRegister.create(IAbility.REGISTRY_KEY,                 ArsMagicaAPI.MOD_ID);
    ...
    static void init(IEventBus bus) {
        //TODO https://github.com/jaredlll08/MultiLoader-Template/blob/1.18.2/examples/MultiLoader-Template-with-registration/Forge/src/main/java/com/example/examplemod/ForgeRegistrationFactory.java
    ...
    private static <T> void registerSimple(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle) {
        internalRegister(key, new MappedRegistry<>(key, lifecycle, null), lifecycle);
    }

    private static <T> void registerDefaulted(ResourceKey<? extends Registry<T>> key, String defaultKey, Lifecycle lifecycle) {
        internalRegister(key, new DefaultedRegistry<>(defaultKey, key, lifecycle, null), lifecycle);
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    private static <T, R extends WritableRegistry<T>> void internalRegister(ResourceKey<? extends Registry<T>> key, R registry, Lifecycle lifecycle) {
        ((WritableRegistry<WritableRegistry<?>>)Registry.REGISTRY).register((ResourceKey)key, registry, lifecycle);
    }
}
