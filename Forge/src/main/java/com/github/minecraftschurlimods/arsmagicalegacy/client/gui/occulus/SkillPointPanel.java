...
public class SkillPointPanel extends Screen implements NarratableEntry {
    ...
    public void render(PoseStack stack, int pMouseX, int pMouseY, float pPartialTicks) {
        ...
        List<Pair<MutableComponent, Integer>> skillPoints = api.getSkillPointRegistry().stream().map(point -> Pair.of(point.getDisplayName().copy().append(new TextComponent(" : " + api.getSkillHelper().getSkillPoint(player, point))), point.color())).toList();
