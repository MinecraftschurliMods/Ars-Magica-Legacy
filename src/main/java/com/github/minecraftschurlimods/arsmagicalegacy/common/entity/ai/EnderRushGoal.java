...
public class EnderRushGoal extends AbstractBossGoal<EnderGuardian> {
    ...
    public void performTick() {
        ...
        if (a.distanceToSqr(b) > 4) {
            ...
            boss.teleportTo(movement.x(), movement.y(), movement.z());
        } else if (boss.level() instanceof ServerLevel level) {
            if (boss.getTarget().hurtServer(level, boss.damageSources().mobAttack(boss), 8)) {
                boss.heal(4);
            }
        }
    }
