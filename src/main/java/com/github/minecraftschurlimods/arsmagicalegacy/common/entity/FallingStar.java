...
public class FallingStar extends Entity implements ItemSupplier {
    ...
    public void tick() {
        ...
        if (timeSinceImpact == -1) {
            ...
            for (int i = 0; i < 8; i++) {
                level.addParticle(PARTICLE, position().x() + random.nextDouble(0.1), position().y() + (i - 4) * Math.abs(getDeltaMovement().y()) / 8f, position().z() + random.nextDouble(0.1), 0, -0.1, 0);
            }
        ...
        for (int i = 0; i < getDamage() * 24; i++) {
            ...
            level.addParticle(PARTICLE, position().x() + random.nextDouble(0.1) + offset.x(), position().y() + 1.5 - random.nextDouble(2), position().z() + random.nextDouble(0.1) + offset.z(), 0, 0, 0);
        }
