...
public class LightningRodGoal extends AbstractBossGoal<LightningGuardian> {
    ...
    public void tick() {
        ...
        if (ticks <= 10 || startPos == null) {
            ...
            target.teleportTo(startPos.x(), startPos.y() + 6, startPos.z());
            if (ticks > 50 && level instanceof ServerLevel serverLevel) {
                target.hurtServer(serverLevel, target.damageSources().lightningBolt(), 20);
            }
            if (!level.isClientSide() && ticks % 20 == 0) {
