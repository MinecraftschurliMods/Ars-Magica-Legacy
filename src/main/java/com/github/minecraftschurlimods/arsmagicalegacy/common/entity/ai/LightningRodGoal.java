...
public class LightningRodGoal extends Goal {
    ...
    public void tick() {
        ...
        if (ticks <= 25) {
            ...
            if (target != null) {
                startPos = new Vector3f((float) target.getX(), (float) target.getY(), (float) target.getZ());
            }
        } else {
            ...
            if (ticks > 85 && ticks <= 150) {
                if (!lightningGuardian.getLevel().isClientSide() && ticks % 20 == 0) {
                    lightningGuardian.getLevel().playSound(null, lightningGuardian, AMSounds.LIGHTNING_GUARDIAN_LIGHTNING_ROD.get(), SoundSource.HOSTILE, 1.0f, lightningGuardian.getRandom().nextFloat() * 0.5f + 0.5f);
                }
            }
            if (ticks <= 85) {
                lightningGuardian.teleportTo(startPos.x(), startPos.y() + ((ticks - 25) * 0.1), startPos.z());
                target.setNoGravity(true);
                if (!lightningGuardian.getLevel().isClientSide() && ticks == 30) {
                    lightningGuardian.getLevel().playSound(null, lightningGuardian, AMSounds.LIGHTNING_GUARDIAN_LIGHTNING_ROD.get(), SoundSource.HOSTILE, 1.0f, lightningGuardian.getRandom().nextFloat() * 0.5f + 0.5f);
                }
            } else if (ticks <= 105) {
                ...
                // Particle
                if (!lightningGuardian.getLevel().isClientSide() && ticks % 20 == 0) {
                    lightningGuardian.getLevel().playSound(null, lightningGuardian, AMSounds.LIGHTNING_GUARDIAN_AMBIENT.get(), SoundSource.HOSTILE, 1.0f, lightningGuardian.getRandom().nextFloat() * 0.5f + 0.5f);  // is LIGHTNING_GUARDIAN_IDLE --> LIGHTNING_GUARDIAN_AMBIENT
                }
            } else if (ticks <= 158 && !hasThrown) {
                target.push(0, -3, 0);
                ...
                hasBolted = true;
                LightningBolt bolt = new LightningBolt(EntityType.LIGHTNING_BOLT, lightningGuardian.getLevel());
                bolt.setPos(target.getX(), target.getY(), target.getZ());
                lightningGuardian.getLevel().addFreshEntity(bolt);
            }
        }
    }
}
