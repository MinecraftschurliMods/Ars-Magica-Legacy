...
public class ManaRegenEffect extends AMMobEffect {
    ...
    public void startEffect(LivingEntity entity, MobEffectInstance effect) {
        Objects.requireNonNull(entity.getAttributes().getInstance(AMAttributes.MANA_REGEN.get())).addTransientModifier(new AttributeModifier(UUID, "burnout_reduction_mob_effect", (effect.getAmplifier() + 1) * 0.5, AttributeModifier.Operation.MULTIPLY_TOTAL));
    }
