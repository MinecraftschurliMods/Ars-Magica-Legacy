...
public class Repel extends AbstractComponent {
    ...
    public SpellCastResult invoke(ISpell spell, LivingEntity caster, Level level, List<ISpellModifier> modifiers, EntityHitResult target, int index, int ticksUsed) {
        ...
        for (Entity e : level.getEntities(entity, entity.getBoundingBox().inflate(range, range, range))) {
            ...
            success = true;
            performRepel(caster, entity.position(), e, modifiers, spell, target);
        }
    ...
    public SpellCastResult invoke(ISpell spell, LivingEntity caster, Level level, List<ISpellModifier> modifiers, BlockHitResult target, int index, int ticksUsed) {
        ...
        for (Entity e : level.getEntities(null, AABB.ofSize(target.getLocation(), range, range, range))) {
            if (e == caster) continue;
            success = true;
            performRepel(caster, target.getLocation(), e, modifiers, spell, target);
        }
        return success ? SpellCastResult.SUCCESS : SpellCastResult.EFFECT_FAILED;
    }

    private static void performRepel(LivingEntity caster, Vec3 targetPos, Entity entity, List<ISpellModifier> modifiers, ISpell spell, HitResult target) {
