...
class CapStateMatcher implements IStateMatcher {
    ...
    CapStateMatcher() {
        this.predicate = (blockGetter, blockPos, state) -> getRegistry(blockGetter, AltarCapMaterial.REGISTRY_KEY).stream().anyMatch(mat -> state.is(mat.cap()));
    }

    @Override
    public BlockState getDisplayedState(long ticks) {
        Registry<AltarCapMaterial> registry = getRegistry(null, AltarCapMaterial.REGISTRY_KEY);
