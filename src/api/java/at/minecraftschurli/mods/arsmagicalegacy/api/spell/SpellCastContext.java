...
/// @param statMultiplier A multiplier to generic stats.
public record SpellCastContext(Spell spell, Level level, @Nullable LivingEntity caster, @Nullable Entity directEntity, @Nullable HitResult hitResult, boolean consume, boolean awardXp, double statMultiplier) {
    /// The context of a spell cast, passed into many spell casting methods in [SpellHelper] and related places.
    ///
    ...
    /// @param awardXp        Whether to award xp or not.
    /// @param statMultiplier A multiplier to generic stats.
    public SpellCastContext(Spell spell, Level level, @Nullable LivingEntity caster, boolean consume, boolean awardXp, double statMultiplier) {
        this(spell, level, caster, null, null, consume, awardXp, statMultiplier);
    }

    /// @param spell The [Spell] to set.
    /// @return A new context object with the [Spell] set.
    public SpellCastContext setSpell(Spell spell) {
        return spell == this.spell ? this : new SpellCastContext(spell, level, caster, directEntity, hitResult, consume, awardXp, statMultiplier);
    }
    ...
    public SpellCastContext setDirectEntity(Entity directEntity) {
        return new SpellCastContext(spell, level, caster, directEntity, hitResult, consume, awardXp, statMultiplier);
    }
